...
const executeCurlCommand = (url: string) =>
  executeCommand(
    `
      curl -Ls '${url}' \
...
const executeCurlCommandOnCambridgeDictionary = (word: string) =>
  executeCurlCommand(`https://dictionary.cambridge.org/dictionary/english/${word}`);

const executeCurlCommandOnMerriamWebster = (word: string) =>
  executeCurlCommand(`https://www.merriam-webster.com/dictionary/${word}`);
...
export const getWordDetailsFromDictionary = async (string: string) => {
  ...
  const merriamWebsterResponse = await executeCurlCommandOnMerriamWebster(parsedWord);
  const cambridgeDictionaryResponse = await executeCurlCommandOnCambridgeDictionary(parsedWord);
